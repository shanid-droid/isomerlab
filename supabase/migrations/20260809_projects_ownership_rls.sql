-- ============================================================================
-- ISOMER MIGRATION: PROJECT OWNERSHIP & PER-ADMIN ISOLATION RLS POLICIES
-- ============================================================================

-- 1. Ensure created_by column exists on public.projects table
ALTER TABLE public.projects 
  ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id) ON DELETE CASCADE DEFAULT auth.uid();

-- 2. Backfill existing projects to Owner UUID (9d5d6287-1843-4cd0-afee-fc1830411571) if unassigned
...
-- 5. Clean up legacy policies on public.projects
...
CREATE POLICY "Admins insert own projects"
  ...
  WITH CHECK (
    public.is_admin() AND (created_by = auth.uid() OR created_by IS NULL)
  );
...
-- 7. Clean up legacy policies on public.project_gallery
