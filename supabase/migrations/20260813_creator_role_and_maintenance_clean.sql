-- ISOMER: Creator role, applications, maintenance mode
-- Prerequisites: profiles, projects, project_gallery, activity_logs,
-- insert_activity_log(), is_owner(), is_admin()
...
-- 2. is_creator helper (does not replace is_owner / is_admin)
...
CREATE POLICY "Users insert own creator application"
  ...
  WITH CHECK (
    ...
    AND NOT EXISTS (
      ...
      WHERE ca.user_id = auth.uid() AND ca.status = 'pending'
    )
  );
...
BEGIN
  PERFORM public.insert_activity_log(
    ...
    'creator_application',
    NEW.id::text,
    jsonb_build_object('full_name', NEW.full_name, 'profession', NEW.profession)
  );
  RETURN NEW;
END;
$fn$;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = NEW.created_by AND role = 'creator'
  ) THEN
    RETURN NEW;
  END IF;
  PERFORM public.insert_activity_log(
...
-- 10. RPC review_creator_application
...
-- 12. RPC sync_creator_requirement_status
