...
const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    // On success, browser navigates away — no further action needed here
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authError) {
      await logAuthEvent('failed_login', { email: email.trim(), method: 'password' });
    ...
    if (data.session?.user) {
      await logAuthEvent('user_login', { email: data.session.user.email ?? email.trim(), method: 'password' });
